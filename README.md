...
This is a sample app that shows the problem when you building an android app that depends on the `androidx.benchmark` and try to run it.
The problem is: classes from package `com.squareup.wire` (and also some others) the app depends on are missing from the final APK.

There different steps to reproduce that results to the similar but different runtime crashes:
...
My first guess was that the jars that makred as `runtime` deps are not passed to the r8 and thus are not bundled to the resulting APK, but the 
